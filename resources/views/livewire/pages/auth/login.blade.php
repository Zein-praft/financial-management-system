<x-guest-layout title="Login - FinTrack">
    <!-- Fonts & Icons -->
        ...
        <div class="w-full max-w-[1000px] bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl rounded-[2.5rem] shadow-[0_20px_50px_-12px_rgba(0,0,0,0.15)] overflow-hidden flex flex-col md:flex-row min-h-[700px] relative z-10 border border-white/50 dark:border-white/5 transition-all duration-500">
            ...
            <div class="w-full md:w-5/12 
                ...
                <div class="relative z-10">
                    ...
                    <p class="text-indigo-100 text-sm md:text-base leading-relaxed font-medium opacity-90">
                        Platform all-in-one yang dirancang untuk mengontrol alur kas bisnis Anda dengan akurasi dan kecepatan tinggi.
                    </p>
                </div>

                <!-- Bagian Bawah: Statistik (STATIS - Tidak Bergerak) -->
                <div class="relative z-10 mt-8">
                    <div class="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-5">
                        <div class="flex items-center gap-3 mb-2">
                            <i class="ph-fill ph-chart-line-up text-2xl text-emerald-400"></i>
                            <span class="font-bold text-lg">Tingkatkan Omzet</span>
                        </div>
                        <p class="text-xs text-indigo-100 leading-relaxed">
                            Bisnis yang menggunakan FinTrack melihat peningkatan efisiensi hingga 40% dalam 3 bulan pertama.
                        </p>
                    </div>
                </div>
            ...
            <div class="w-full md:w-7/12 bg-white dark:bg-slate-900 p-8 md:p-12 flex flex-col justify-center relative">
                ...
                <div class="mb-10 max-w-md">
                    <h2 class="font-heading text-3xl font-bold text-slate-900 dark:text-white mb-2">Selamat Datang</h2>
                    <p class="text-slate-500 dark:text-slate-400 text-sm">Silakan masukkan kredensial akun Anda untuk mengakses dashboard.</p>
                </div>
                ...
                <form method="POST" action="{{ route('login') }}" class="space-y-6">
                    ...
                    <div class="flex items-center justify-between text-sm font-medium">
                        <label class="flex items-center gap-3 cursor-pointer text-slate-600 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors select-none">
                            <input type="checkbox" name="remember" class="w-4 h-4 rounded text-indigo-600 focus:ring-indigo-500 border-slate-300 dark:border-slate-600 bg-transparent"> 
                            <span class="text-xs font-bold">Ingat Saya</span>
                        </label>
                        @if (Route::has('password.request'))
                            <a href="{{ route('password.request') }}" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 font-semibold hover:underline">Lupa Password?</a>
                        @endif
                    </div>
                    ...
                    <button type="submit"
                        ...
                        <span class="relative z-10 flex items-center justify-center gap-2 text-lg">
                            Masuk ke Dashboard <i class="ph-bold ph-arrow-right group-hover:translate-x-1 transition-transform"></i>
                        </span>
                ...
                <div class="mt-8 text-center text-sm text-slate-500 dark:text-slate-400 font-medium">
                    Belum punya akun? 
                    <a href="{{ route('register') }}" class="text-indigo-600 dark:text-indigo-400 font-bold hover:text-indigo-500 hover:underline ml-1">Buat Akun Baru</a>
                </div>
